#!/usr/bin/env node
/**
 * generate-permission-golang.js - Go Permission Keys Code Generation Script
 *
 * DESCRIPTION:
 *   Generates Go constants for permission keys from a CSV source.
 *   Each constant is named using the Category (Theme) + Function columns in PascalCase,
 *   with the Key ID (UUID) as the value and Feature as a comment.
 *
 * WHAT IT DOES:
 ...
 *   3. Generates Go constants with proper naming conventions
 *   4. Outputs to the specified location (default: models/permissions.go)
 *
 * USAGE:
 *   node build/generate-permission-golang.js [options]
 *
 * OPTIONS:
 *   --source, -s    CSV source: local file path, URL, or Google Sheets URL
 *                   (default: permissions.csv)
 *   --output, -o    Output file path (default: models/permissions.go)
 ...
 *   node build/generate-permission-golang.js --source ./data/permissions.csv
 *
 *   # Download from Google Sheets
 *   node build/generate-permission-golang.js --source "https://docs.google.com/spreadsheets/d/SHEET_ID/edit"
 *
 *   # Specify output location
 *   node build/generate-permission-golang.js --output ./pkg/permissions/keys.go
 *
 * OUTPUT:
...
const DEFAULT_OUTPUT = "models/permissions/permissions.go";
...
function printHelp() {
  console.log(`
Usage: node build/generate-permission-golang.js [options]
...
async function main() {
  ...
  try {
    ...
    logger.blank();
    logger.success(`Go permission keys generation complete! (${duration}s)`);
