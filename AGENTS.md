...
Referencing the full entity schema in a `requestBody` is WRONG - it forces clients to supply server-generated fields. Reference the `{Construct}Payload` schema instead.

Worked YAML examples for all three rules (entity schema, payload schema, WRONG vs CORRECT requestBody) live in [Dual-schema pattern examples](docs/schema-authoring-reference.md#dual-schema-pattern-examples).
...
Before opening a PR that adds an entity schema, walk the "(New entity)" and "(New endpoint)" items in the
[Checklist for Schema Changes](docs/schema-review-checklist.md#checklist-for-schema-changes).
...
> (canonical URL: <https://github.com/meshery/schemas/blob/master/docs/identifier-naming-contributor-guide.md>).

**Wire is camelCase everywhere; DB is snake_case; Go fields follow Go idiom; the ORM layer is the sole translation boundary.**

This repo IS the authority for that contract. The contract is not optional; deviations block PRs via the schemas consumer-audit CI gate. On any conflict, schemas wins - downstream repos file discrepancies as issues against `meshery/schemas`, not locally.
...
- Endpoint paths: kebab-case, plural nouns, and the `/api` prefix that the authenticated API surface is served under (`/api/workspaces`, `/api/environments`). The declared path must **equal the route the server serves** - nothing prepends `/api` at build time, so a path that does not match ships a client that 404s at runtime without failing any build or validation step. Carry `/api` only where the router group actually applies it: `/user/schedules` is registered on the bare router and is correct without it. Verify against the consumer's route registration (Echo groups supply the prefix); see [`docs/http-api-design.md`](docs/http-api-design.md#declared-paths-are-absolute---there-is-no-implicit-api-prefix).
...
Every construct directory `schemas/constructs/<version>/<construct>/` holds `api.yml` (OpenAPI spec: endpoints + all schema definitions), `<construct>.yaml` (entity/response schema), `templates/` (example instances), and optional `forms/` (RJSF form schemas). Full layout: [File structure for a construct](docs/schema-authoring-reference.md#file-structure-for-a-construct).
...
**Use the `meshery-schemas-release` skill** (`/meshery-schemas-release`) to cut releases - it automates the entire procedure and includes safeguards.
**CRITICAL: Do not manually create releases** - no `gh release create`, no manual `npm publish`, no hand-edited release notes, no manual version bumps.
The chain: push to `master` -> `generate-artifacts-from-schemas.yml` self-commits generated artifacts -> `release-drafter.yml` refreshes the draft release -> a human publishes the draft -> publish workflows fan out (`publish-schemas.yml`, npm publish, downstream notifications, OpenAPI docs).
Full procedure, the What NOT to Do list, and versioning policy: [`docs/release-procedure.md`](docs/release-procedure.md). Skill usage and troubleshooting: [`docs/release-procedure-skill.md`](docs/release-procedure-skill.md).

## Detailed references

Long-form reference material lives in `docs/` to keep this file concise. Consult these when you need the full detail:
