...
## x-internal / x-annotation annotations

**`x-internal` is required on every operation.** The bundler and validate-schemas Rule 14 reject operations that omit it. Use it to declare which bundled outputs include the path:
...
**`x-annotation` is required on every construct and every operation.** Use `info.x-annotation` plus operation-level `x-annotation` to track which downstream project supports the construct or endpoint:
