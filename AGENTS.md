...
- Endpoint paths: `/api` prefix, kebab-case, plural nouns (`/api/workspaces`, `/api/environments`). The declared path is the **full served path** - nothing prepends `/api` at build time, so a missing prefix ships a client that 404s at runtime without failing any build or validation step. Verify against the consumer's route registration (Echo groups supply the prefix); see [`docs/http-api-design.md`](docs/http-api-design.md#declared-paths-are-absolute---there-is-no-implicit-api-prefix).
