...
# oapi-codegen
# Not installed here on purpose. The generator runs `go tool oapi-codegen`, so
# the version comes from the `tool` directive in go.mod and nowhere else. A
# `go install` of a second copy would put an unpinned binary on PATH and let a
# contributor regenerate every committed model with an unrelated diff.
