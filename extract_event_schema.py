...
target_dir_path = "schemas/constructs/event/v1beta1/"
target_file_path = os.path.join(target_dir_path, "event.json")
event_schema_json = None
error_message = None
file_existed = False

try:
    data = yaml.safe_load(definitions_yaml_content)
    event_schema = data.get('components', {}).get('schemas', {}).get('event')

    if not event_schema:
        error_message = "Error: 'Event' schema not found in definitions_yaml_content."
    else:
        event_schema_json = json.dumps(event_schema, indent=2)

except yaml.YAMLError as e:
    error_message = f"Error parsing YAML: {e}"
except Exception as e:
    error_message = f"An unexpected error occurred during YAML parsing or JSON conversion: {e}"

# Output results for the calling agent to process
if error_message:
    print(error_message, file=sys.stderr)
    # To ensure the script doesn't try to proceed with file creation if schema extraction failed
    event_schema_json = None 

if event_schema_json:
    print("---EVENT_SCHEMA_START---")
    print(event_schema_json)
    print("---EVENT_SCHEMA_END---")

# The rest of the logic (file checking, creation) will be handled by the agent using tools.
# This script now solely focuses on extracting and printing the schema or an error.
```
