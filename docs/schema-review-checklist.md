...
- [ ] (New or changed write endpoint) The `*Payload` carries exactly the fields the handler reads off the body - fields the server pins from the stored row are excluded, not declared-and-ignored - and when the write replaces rather than merges, every unpinned writable field is `required` so omitting one cannot silently zero a column
- [ ] (New or changed endpoint) Denial responses are read from the **route registration** as well as the handler: a route wrapped in organization-authorization middleware declares `403`, one without it does not, and a gate that answers a uniform `404` for both "not yours" and "not there" says so in the operation `description`
