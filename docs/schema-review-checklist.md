...
- [ ] (New or changed endpoint) The success response schema matches the bytes the consumer's handler actually writes - verified against the handler, not assumed. If the handler marshals a slice, the schema is `type: array` with `items` `$ref`-ing the canonical entity schema; a `{ message: string }` envelope is only correct when the handler really writes one
