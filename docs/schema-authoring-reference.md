...
## Dual-schema pattern examples

Worked examples for the three dual-schema rules stated in `AGENTS.md § The Dual-Schema Pattern (REQUIRED)`.

### Rule 1 example: `<construct>.yaml` = response schema only
...
### Rule 2 example: `{Construct}Payload` schema in `api.yml`
...
### Rule 3 example: POST/PUT requestBody references `*Payload`, never the entity schema
